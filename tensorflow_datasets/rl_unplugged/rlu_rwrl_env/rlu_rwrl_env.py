...
Specifically, every timestep contains the following features:

* o_t: Observation at time t. A dictionary.
* a_t: Action taken at time t after the agent has seen o_t. The last action
    will be a placeholder value.
* r_t: Reward at time t that the agent got from the environment after taking the
    action a_t. The last reward will be a placeholder value.
...
SHARDS_MAPPING = {
    # ('combined_challenge','domain','task','dataset_size'): number of shards.
    (None, 'cartpole', 'swingup', '1_percent'):
        3,
    (None, 'cartpole', 'swingup', '5_percent'):
        3,
    (None, 'cartpole', 'swingup', '20_percent'):
        6,
    (None, 'cartpole', 'swingup', '40_percent'):
        6,
    (None, 'cartpole', 'swingup', '100_percent'):
        7,
    (None, 'quadruped', 'walk', '1_percent'):
        3,
    (None, 'quadruped', 'walk', '5_percent'):
        3,
    (None, 'quadruped', 'walk', '20_percent'):
        4,
    (None, 'quadruped', 'walk', '40_percent'):
        7,
    (None, 'quadruped', 'walk', '100_percent'):
        10,
    (None, 'walker', 'walk', '1_percent'):
        3,
    (None, 'walker', 'walk', '5_percent'):
        6,
    (None, 'walker', 'walk', '20_percent'):
        14,
    (None, 'walker', 'walk', '40_percent'):
        48,
    (None, 'walker', 'walk', '100_percent'):
        94,
    (None, 'humanoid', 'walk', '1_percent'):
        9,
    (None, 'humanoid', 'walk', '5_percent'):
        123,
    (None, 'humanoid', 'walk', '20_percent'):
        125,
    (None, 'humanoid', 'walk', '40_percent'):
        344,
    (None, 'humanoid', 'walk', '100_percent'):
        391,
    ('easy', 'cartpole', 'swingup', '1_percent'):
        2,
    ('easy', 'cartpole', 'swingup', '5_percent'):
        3,
    ('easy', 'cartpole', 'swingup', '20_percent'):
        8,
    ('easy', 'cartpole', 'swingup', '40_percent'):
        8,
    ('easy', 'cartpole', 'swingup', '100_percent'):
        14,
    ('easy', 'quadruped', 'walk', '1_percent'):
        3,
    ('easy', 'quadruped', 'walk', '5_percent'):
        3,
    ('easy', 'quadruped', 'walk', '20_percent'):
        9,
    ('easy', 'quadruped', 'walk', '40_percent'):
        10,
    ('easy', 'quadruped', 'walk', '100_percent'):
        15,
    ('easy', 'walker', 'walk', '1_percent'):
        11,
    ('easy', 'walker', 'walk', '5_percent'):
        17,
    ('easy', 'walker', 'walk', '20_percent'):
        78,
    ('easy', 'walker', 'walk', '40_percent'):
        137,
    ('easy', 'walker', 'walk', '100_percent'):
        371,
    ('easy', 'humanoid', 'walk', '1_percent'):
        18,
    ('easy', 'humanoid', 'walk', '5_percent'):
        98,
    ('easy', 'humanoid', 'walk', '20_percent'):
        244,
    ('easy', 'humanoid', 'walk', '40_percent'):
        410,
    ('easy', 'humanoid', 'walk', '100_percent'):
        769,
}
...
def _builder_configs():
  ...
  for domain in DOMAIN_NAMES:
    ...
    for combined_challenge in COMBINED_CHALLENGES:
...
class RluRwrlEnv(rlu_common.RLUBuilder):
  """DatasetBuilder for rlu_rwrl_env dataset."""
  ...
  def num_shards(self):
    ...
    combined_challenge = self.builder_config.combined_challenge
    dataset_size = self.builder_config.dataset_size
    return SHARDS_MAPPING[(combined_challenge, domain, task, dataset_size)]
  ...
  def tf_example_to_step_ds(self, tf_example: tf.Tensor,
    ...
    episode_length = tf.size(data['action'])
    is_first = tf.concat([[True], [False] * tf.ones(episode_length - 1)],
                         axis=0)
    is_terminal = tf.squeeze(data['discount'] == 0., axis=-1)
    episode_return = tf.reduce_sum(data['reward'])
    episode = {
        # Episode Metadata
